...
def test_function_scope_isolation():
    """Test that variables in different functions don't conflict"""
    ...
    def shader(vs_uv: vec2) -> vec4:
        ...
        def square(p: vec2, s: float) -> float:
            d = max(abs(p.x), abs(p.y))
            return d - s

        c = circle(vs_uv, 0.5)
        s = square(vs_uv, 0.4)
        return vec4(min(c, s), 0.0, 0.0, 1.0)

    result = py2glsl(shader)
    # Each function should have its own 'd' variable
    assert result.fragment_source.count("float d;") == 2
