...
def test_normalize_type_error():
    ...
    try:
        ...
        try:
            ...
            assert "int" in error_msg, "Should mention the int type"

            # Verify some kind of location info is included
            has_location = (
                "line" in error_msg
                or "file" in error_msg
                or "test_shader" in error_msg
            )
            assert has_location, "Should include location information"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            helper_spec = importlib.util.spec_from_file_location(
                "helper_module", helper_path
            )
            helper_module = importlib.util.module_from_spec(helper_spec)
            helper_spec.loader.exec_module(helper_module)

            shader_spec = importlib.util.spec_from_file_location(
                "test_shader", shader_path
            )
            ...
            # Let's try to generate GLSL code instead of checking for errors

            try:
                # Try to transpile, should either generate code or raise an error
                glsl_code, _ = transpile(
                    shader_module.shader,
                    helper_module.normalize_vector,
                    helper_module.create_vector
                )
